...
\begin{chapter}{\texorpdfstring{He\textsuperscript{+}}{He+}-He Collisions \label{chap:hephe}}
   ...
   \begin{section}{Final-state analysis \label{sec:hephe-det}}
      ...
      \end{equation}
      %
      with $f$ and $f`$ $\in \{f_1, f_2, f_3\}$, and the transition amplitudes $\braket{f |
      \varphi_{\sigma j}(t_f)} = \braket{\tilde{\chi}^{l0}_{c k} | \varphi_{\sigma j}(t_f)}$ (for some
      $k$, $l$, $c$, and properly orthogonalized basis functions $\tilde{\chi}^{l0}_{c k}$) readily
      calculable from the dynamics. A model of this type which ignores the functional
